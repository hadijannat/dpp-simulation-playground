...
def create_run(request: Request, payload: RunCreateRequest, db: Session = Depends(get_db)):
...
def get_run(request: Request, run_id: str, db: Session = Depends(get_db)):
    ...
    run = data["run"]
    template = db.query(journey_repo.JourneyTemplate).filter_by(id=run.template_id).first()
    return _serialize_run(run, data["step_runs"], template_code=template.code if template else "")
...
def execute_step(
    ...
    step_run = journey_repo.create_step_run(
        db, run_id=run.id, step_key=step_id, payload=payload.payload, metadata=payload.metadata
    )
    ...
    return {
        ...
        "execution": {
            ...
            "executed_at": step_run.executed_at.isoformat() if step_run.executed_at else "",
        },
