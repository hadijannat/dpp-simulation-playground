...
test.describe("Settings Language Switching", () => {
  test("language selection persists across page reload", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /einstellungen/i })).toBeVisible({ timeout: 5_000 });

    await page.reload();
    await expect(page.getByRole("heading", { name: /einstellungen/i })).toBeVisible({ timeout: 5_000 });

    await page.locator("main").getByRole("combobox").selectOption("fr");
    await expect(page.getByRole("heading", { name: /paramètres/i })).toBeVisible({ timeout: 5_000 });
